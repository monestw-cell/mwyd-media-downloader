...
//        }
    ...
    private suspend fun getYoutubeVideo(inputQuery: String, resetResults: Boolean, addToResults: Boolean) : List<ResultItem>{
        val theURL = inputQuery.replace("\\?list.*".toRegex(), "").replace("&list=.*".toRegex(), "")
        val newpipeExtractorResult = newPipeUtil.getVideoData(theURL)

        val res = if (newpipeExtractorResult.isSuccess && newpipeExtractorResult.getOrNull()?.isNotEmpty() == true) {
            val list = newpipeExtractorResult.getOrNull()!!
            list.forEach { it.url = inputQuery }
            list
        }else{
            val fast = newPipeUtil.fetchFastOEmbedVideo(theURL)
            if (fast != null) {
                fast.url = inputQuery
                listOf(fast)
            } else {
        ...
        if (resetResults) {
            ...
            res.filter { it.playlistTitle.isBlank() }.forEach { it.playlistTitle = MWYD_SEARCH }
        }
        if (addToResults){
            val ids = resultDao.insertMultiple(res)
            ids.forEachIndexed { index, id ->
    ...
    private suspend fun getFromYTDLP(inputQuery: String, resetResults: Boolean, addToResults: Boolean, singleItem: Boolean = false) : List<ResultItem> {
        ...
        ytdlpUtil.getFromYTDL(inputQuery, singleItem) { results ->
            ...
            results.filter { it.playlistTitle.isBlank() }.forEach { it.playlistTitle = MWYD_SEARCH }
            if (addToResults) {
    ...
    fun getFormats(url: String, source : String? = null) : List<Format> {
        ...
        val res = if (url.isYoutubeURL()) {
            if (formatSource == "yt-dlp") {
                val ytdlpFormats = ytdlpUtil.getFormats(url)
                if (ytdlpFormats.isNotEmpty()) return ytdlpFormats
                NewPipeUtil(context).getFormats(url)
            } else {
                val tmpRes = NewPipeUtil(context).getFormats(url)
                if (tmpRes.isSuccess && tmpRes.getOrNull()?.isNotEmpty() == true) {
                    tmpRes
                } else {
                    val ytdlpFormats = ytdlpUtil.getFormats(url)
                    if (ytdlpFormats.isNotEmpty()) Result.success(ytdlpFormats) else Result.failure(Throwable())
                }
            }
        } else {
            Result.failure(Throwable())
    ...
    suspend fun updateDownloadItem(
        ...
        if (downloadItem.needsDataUpdating()){
            runCatching {
                ...
                if (downloadItem.playlistTitle.isEmpty() || (downloadItem.playlistTitle.isNotBlank() && downloadItem.playlistTitle != MWYD_SEARCH)   ) downloadItem.playlistTitle = info.playlistTitle
