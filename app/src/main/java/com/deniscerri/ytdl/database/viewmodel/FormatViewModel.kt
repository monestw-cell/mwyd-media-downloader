...
class FormatViewModel(private val application: Application) : AndroidViewModel(application) {
    ...
    var filterBy = MutableStateFlow(FormatCategory.valueOf(sharedPreferences.getString("format_filter", "SUGGESTED")!!))
    ...
    init {
        ...
        formats = combine(listOf(selectedItemsSharedFlow, filterBy)) { f ->
            ...
            if (items.isEmpty()) {
                ...
                when(filterBy.value){
                    FormatCategory.ALL -> {}
                    FormatCategory.SUGGESTED, FormatCategory.SMALLEST -> {
                        val videoSimplified = formatUtil.getSimplifiedVideoFormats(finalFormats)
                        val audioSimplified = formatUtil.getSimplifiedAudioFormats(finalFormats)
                        if (videoSimplified.isNotEmpty() || audioSimplified.isNotEmpty()) {
                            finalFormats = videoSimplified + audioSimplified
                        } else {
