...
class FormatUtil(private var context: Context) {
    ...
    fun getSimplifiedVideoFormats(formats: List<Format>): List<Format> {
        val videoCandidates = formats.filter { f ->
            val hasVideo = f.vcodec.isNotBlank() && f.vcodec != "none"
            hasVideo && !f.format_note.contains("storyboard", ignoreCase = true)
        }
        if (videoCandidates.isEmpty()) return emptyList()

        val sorted = sortVideoFormats(videoCandidates)
        val groupedByHeight = sorted.filter { (it.height ?: 0) > 0 }.groupBy { it.height }

        val simplified = mutableListOf<Format>()
        groupedByHeight.keys.filterNotNull().sortedDescending().forEach { h ->
            val bestForHeight = groupedByHeight[h]?.firstOrNull()
            if (bestForHeight != null) {
                val fmtCopy = bestForHeight.copy(
                    format_note = "${h}p"
                )
                simplified.add(fmtCopy)
            }
        }

        val noHeight = sorted.filter { (it.height ?: 0) <= 0 }
        if (simplified.isEmpty() && noHeight.isNotEmpty()) {
            simplified.addAll(noHeight.take(3))
        }

        return simplified
    }

    fun getSimplifiedAudioFormats(formats: List<Format>): List<Format> {
        val audioCandidates = formats.filter { f ->
            val isAudioOnly = (f.vcodec.isBlank() || f.vcodec == "none") &&
                    (f.acodec.isNotBlank() && f.acodec != "none" || f.format_note.contains("audio", ignoreCase = true))
            isAudioOnly && !f.format_note.contains("storyboard", ignoreCase = true)
        }
        if (audioCandidates.isEmpty()) return emptyList()

        val sorted = sortAudioFormats(audioCandidates)

        fun parseBitrate(f: Format): Int {
            ...
            if (noteMatch != null) return noteMatch.groupValues[1].toIntOrNull() ?: 0
            return 0
        }

        val groupedByBitrate = sorted.groupBy { parseBitrate(it) }
        val simplified = mutableListOf<Format>()

        groupedByBitrate.keys.sortedDescending().forEach { abr ->
            val bestForBitrate = groupedByBitrate[abr]?.firstOrNull()
            if (bestForBitrate != null) {
                val label = if (abr > 0) "$abr kbps" else bestForBitrate.format_note.ifBlank { "Audio" }
                val fmtCopy = bestForBitrate.copy(
                    format_note = label
                )
                simplified.add(fmtCopy)
            }
        ...
        return simplified.ifEmpty { sorted.take(3) }
    }
