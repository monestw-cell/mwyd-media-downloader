...
class YTDLPUtil(private val context: Context, private val commandTemplateDao: CommandTemplateDao) {
    ...
    fun getFormats(url: String) : List<Format> {
        ...
        if (!request.hasOption("--no-playlist") && !url.contains("playlist?list=")) request.addOption("--no-playlist")
        if (!request.hasOption("--socket-timeout")) request.addOption("--socket-timeout", "10")
        if (!request.hasOption("--no-check-certificates")) request.addOption("--no-check-certificates")
    ...
    fun buildYTDLRequest(downloadItem: DownloadItem) : YTDLRequest {
        ...
        val retries = sharedPreferences.getString("retries", "")!!.ifBlank { "20" }
        val fragmentRetries = sharedPreferences.getString("fragment_retries", "")!!.ifBlank { "50" }
        val socketTimeout = sharedPreferences.getString("socket_timeout", "")!!.ifBlank { "30" }

        request.addOption("--retries", retries)
        request.addOption("--fragment-retries", fragmentRetries)
        request.addOption("--retry-sleep", "fragment:exp=1:20")
        request.addOption("--retry-sleep", "exp=1:20")
        request.addOption("--file-access-retries", "10")
        ...
        if(downloadItem.type != DownloadType.command){
            ...
            * */
            ...
            if (canUseWriteInfoJson && downloadItem.playlistURL.isNullOrBlank()) {
                ...
                // If resuming a partial download, don't use stale cached info.json (stream URLs might be expired/forbidden)
                val isResuming = downDir.exists() && (downDir.listFiles()?.any { it.name.endsWith(".part") || it.name.endsWith(".ytdl") } == true)
                if (!isResuming && infoJsonFile != null && System.currentTimeMillis() - infoJsonFile.lastModified() <= (1000 * 60 * 60 * 2)) {
                    request.addOption("--load-info-json", infoJsonFile.absolutePath)
                }else {
        ...
        when(type){
            DownloadType.audio -> {
                ...
                val preferredLanguage = downloadItem.audioPreferences.audioLanguage.ifBlank {
                    sharedPreferences.getString("audio_language","")!!
                }
                ...
                if (audioQualityId.isBlank() || listOf("0", context.getString(R.string.best_quality), "ba", "best", "", context.getString(
                    ...
                    audioQualityId = "ba/b"
                }else if(audioQualityId.contains("kbps_mwydgeneric")){
                ...
                if ((audioQualityId.isBlank() || audioQualityId == "ba/b") && preferredLanguage.isNotBlank()) {
                    audioQualityId = "ba[language^=$preferredLanguage]/ba[lang^=$preferredLanguage]/${audioQualityId.ifEmpty { "ba/b" }}"
                }
                ...
                if (preferredLanguage.isNotBlank()) {
                    formatSorting.add(0, "lang:${preferredLanguage}")
                }
            ...
            DownloadType.video -> {
                ...
                var cont = if (downloadItem.container.isNotEmpty() && downloadItem.container != "Default" && downloadItem.container != context.getString(R.string.defaultValue) && supportedContainers.contains(downloadItem.container)) {
                ...
                if (downloadItem.videoPreferences.recodeVideo && !cantRecode.contains(cont)) {
                    request.addOption("--recode-video", cont.lowercase())
                } else if (!downloadItem.videoPreferences.compatibilityMode){
                    request.addOption("--merge-output-format", cont.lowercase())
                }

                if (!listOf("webm", "avi", "flv", "gif").contains(cont.lowercase())) {
                    if (downloadItem.videoPreferences.embedThumbnail) {
                        metadataCommands.addOption("--embed-thumbnail")
                        if (!request.toString().contains("--convert-thumbnails")) request.addOption("--convert-thumbnails", thumbnailFormat ?: "jpg")
                    }
                }
                ...
                val preferredAudioLanguage = downloadItem.videoPreferences.audioLanguage.ifBlank {
                    sharedPreferences.getString("audio_language", "")!!
                }
                ...
                if (preferredAudioLanguage.isNotBlank() && !downloadItem.videoPreferences.removeAudio) {
                    audioF = "ba[language^=$preferredAudioLanguage]/ba[lang^=$preferredAudioLanguage]/$audioF/ba"
                }

                if (downloadItem.videoPreferences.removeAudio) audioF = ""

                var abrSort = ""
                if(audioF.contains("kbps_mwydgeneric")){
                ...
                var targetHeight: Int? = downloadItem.format.height?.takeIf { it > 0 }
                if (targetHeight == null) {
                    val genericMatch = """(\d{3,4})p?""".toRegex().find(videoF)
                    if (genericMatch != null) {
                        targetHeight = genericMatch.groupValues[1].toIntOrNull()
                    }
                }

                var hasGenericResulutionFormat = targetHeight?.toString() ?: ""
                val usingGenericFormat = defaultFormats.contains(videoF) || videoF.contains("_mwydgeneric") || videoF.startsWith("bestvideo[") || downloadItem.allFormats.isEmpty() || downloadItem.allFormats == formatUtil.getGenericVideoFormats(context.resources)

                if (hasGenericResulutionFormat.isNotBlank()) {
                    val h = hasGenericResulutionFormat
                    if (downloadItem.videoPreferences.removeAudio || audioF.isBlank()) {
                        f.append("bv*[height<=$h]/bv*[width<=$h]/b[height<=$h]/b[width<=$h]/bv/b")
                    } else {
                        // In generic resolution mode, NEVER use hardcoded format IDs (like 251, 140).
                        // They cause "Requested format is not available" when the specific format
                        // isn't present in the player client's response. Always use 'ba' (best audio).
                        if (preferredAudioLanguage.isNotBlank()) {
                            f.append("bv*[height<=$h]+ba[language^=$preferredAudioLanguage]/")
                            f.append("bv*[width<=$h]+ba[language^=$preferredAudioLanguage]/")
                        }
                        f.append("bv*[height<=$h]+ba/bv*[width<=$h]+ba/bv*+ba/b[height<=$h]/b[width<=$h]/bv*[height<=$h]/bv*/b/best")
                    }
                } else if (!usingGenericFormat){
                    // with audio removed
                    if (audioF.isBlank()){
                    ...
                    preferredFormatIDs.forEach { v ->
                        ...
                        if (!downloadItem.videoPreferences.removeAudio){
                            ...
                            if (preferredAudioLanguage.isNotBlank()){
                                val al = if (v.startsWith("wv") || v.startsWith("w")){
                            ...
                            if (v.startsWith("wv") || v.startsWith("w")){
                                f.append("w/")
                            }else{
                                if (v != "bv") f.append("$v/")
                            }
                ...
                if (hasGenericResulutionFormat.isNotBlank()) {
                    formatSorting.remove("res:${hasGenericResulutionFormat}")
                    formatSorting.add(0, "res:${hasGenericResulutionFormat}")
                }

                if (preferredAudioLanguage.isNotBlank()) {
                    formatSorting.add(0, "lang:${preferredAudioLanguage}")
                }
                ...
                if (downloadItem.videoPreferences.removeAudio && cont != "gif") {
