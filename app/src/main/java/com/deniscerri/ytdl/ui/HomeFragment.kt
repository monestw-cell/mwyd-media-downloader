...
class HomeFragment : Fragment(), HomeAdapter.OnItemClickListener, SearchSuggestionsAdapter.OnItemClickListener, OnClickListener {
    ...
    private var dashboardRootLayout: LinearLayout? = null
    private var searchResultsContainer: LinearLayout? = null
    private var dashboardRecentContainer: LinearLayout? = null
    private var dashboardRecentEmptyCard: View? = null
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        lifecycleScope.launch {
            resultViewModel.totalCount.collectLatest {
                totalCount = it
                updateDashboardVisibility(it > 0 && queryList.isNotEmpty())
            }
        }
        ...
        if (inputQueries != null) {
            ...
            // Clean state on fresh dashboard launch
            lifecycleScope.launch(Dispatchers.IO){
                resultViewModel.deleteAll()
            }
        }
        ...
        searchView?.addTransitionListener { _, _, newState ->
            if (newState == SearchView.TransitionState.SHOWING){
                ...
                if (totalCount == 0 && queryList.isEmpty()) {
    ...
    private fun initDashboard(view: View) {
        dashboardRootLayout = view.findViewById(R.id.dashboard_root_layout)
        searchResultsContainer = view.findViewById(R.id.search_results_container)
        dashboardRecentContainer = view.findViewById(R.id.dashboard_recent_downloads_container)
        dashboardRecentEmptyCard = view.findViewById(R.id.dashboard_recent_empty_card)

        val searchCard = view.findViewById<View>(R.id.dashboard_search_card)
        val searchBtnCard = view.findViewById<View>(R.id.dashboard_search_btn_card)
        val searchCardClickListener = View.OnClickListener {
            searchView?.show()
        }
        searchCard?.setOnClickListener(searchCardClickListener)
        searchBtnCard?.setOnClickListener(searchCardClickListener)

        // 1. Settings Card
        view.findViewById<View>(R.id.dashboard_card_settings)?.setOnClickListener {
            try {
                val intent = Intent(requireContext(), com.deniscerri.ytdl.ui.more.settings.SettingsActivity::class.java)
                startActivity(intent)
            } catch (e: Exception) {
                mainActivity?.findViewById<NavigationBarView>(R.id.bottomNavigationView)?.selectedItemId = R.id.moreFragment
            }
        }

        // 2. Playlist Card
        view.findViewById<View>(R.id.dashboard_card_playlist)?.setOnClickListener {
            handleQuickAction(isPlaylist = true)
        }

        // 3. Audio Extraction Card
        view.findViewById<View>(R.id.dashboard_card_audio)?.setOnClickListener {
            handleQuickAction(preferredType = DownloadType.audio)
        }

        // 4. Quick Download Card
        view.findViewById<View>(R.id.dashboard_card_quick)?.setOnClickListener {
            handleQuickAction(preferredType = DownloadType.video)
        }

        // View all recent downloads
        view.findViewById<View>(R.id.dashboard_view_all_downloads)?.setOnClickListener {
            try {
                mainActivity?.findViewById<NavigationBarView>(R.id.bottomNavigationView)?.selectedItemId = R.id.historyFragment
            } catch (e: Exception) {
                findNavController().navigate(R.id.historyFragment)
            }
        }

        refreshDashboardRecentDownloads()
    }

    private fun handleQuickAction(isPlaylist: Boolean = false, preferredType: DownloadType? = null) {
        val clipboard = requireContext().getSystemService(CLIPBOARD_SERVICE) as ClipboardManager
        val clipText = clipboard.primaryClip?.getItemAt(0)?.text?.toString()?.trim() ?: ""
        if (clipText.isURL()) {
            processQuickUrl(clipText, preferredType)
        } else {
            searchView?.show()
        }
    }

    private fun processQuickUrl(url: String, preferredType: DownloadType?) {
        searchBar?.setText(url)
        lifecycleScope.launch(Dispatchers.IO) {
            resultViewModel.deleteAll()
            resultViewModel.parseQueries(mutableListOf(url)) {
                if (preferredType != null) {
                    val first = resultViewModel.firstResult.value
                    if (first != null) {
                        lifecycleScope.launch(Dispatchers.Main) {
                            showSingleDownloadSheet(first, preferredType)
                        }
                    }
                }
            }
    ...
    private fun refreshDashboardRecentDownloads() {
        lifecycleScope.launch(Dispatchers.IO) {
            val allHistory = historyViewModel.getAll().take(3)
            withContext(Dispatchers.Main) {
                if (!isAdded) return@withContext
                if (allHistory.isEmpty()) {
                    dashboardRecentEmptyCard?.visibility = View.VISIBLE
                    dashboardRecentContainer?.removeAllViews()
                } else {
                    dashboardRecentEmptyCard?.visibility = View.GONE
                    dashboardRecentContainer?.removeAllViews()
                    for (item in allHistory) {
                        val row = layoutInflater.inflate(R.layout.item_dashboard_recent_download, dashboardRecentContainer, false)
                        val titleView = row.findViewById<TextView>(R.id.recent_item_title)
                        val badgeView = row.findViewById<TextView>(R.id.recent_item_format_badge)
                        val sizeView = row.findViewById<TextView>(R.id.recent_item_size)
                        val durationView = row.findViewById<TextView>(R.id.recent_item_duration)
                        val thumbView = row.findViewById<ImageView>(R.id.recent_item_thumb)
                        val moreBtn = row.findViewById<View>(R.id.recent_item_more)

                        titleView.text = item.title
                        badgeView.text = item.type.name.uppercase()
                        sizeView.text = if (item.filesize > 0) com.deniscerri.ytdl.util.FileUtil.convertFileSize(item.filesize) else ""
                        durationView.text = item.duration.ifBlank { "00:00" }

                        thumbView.loadThumbnail(false, item.thumb)

                        val firstPath = item.downloadPath.firstOrNull()
                        row.setOnClickListener {
                            if (firstPath != null) {
                                com.deniscerri.ytdl.util.FileUtil.openFileIntent(requireContext(), firstPath)
                            }
                        }
                        ...
                        dashboardRecentContainer?.addView(row)
                    }
                }
            }
        }
    ...
    private fun updateDashboardVisibility(hasActiveResults: Boolean) {
        if (hasActiveResults) {
            dashboardRootLayout?.visibility = View.GONE
            searchResultsContainer?.visibility = View.VISIBLE
        } else {
            dashboardRootLayout?.visibility = View.VISIBLE
...
//            searchSuggestionsLinearLayout!!.isVisible = suggestions.isNotEmpty()
    ...
    private fun showSingleDownloadSheet(
        ...
        if(findNavController().currentBackStack.value.firstOrNull {it.destination.id == R.id.downloadBottomSheetDialog} == null &&
            ...
            val isPlaylistUrl = resultItem.url.contains("playlist?list=", ignoreCase = true) ||
                    resultItem.url.contains("&list=", ignoreCase = true) ||
                    resultItem.url.contains("/sets/", ignoreCase = true)
            val useQuickMode = sharedPreferences?.getBoolean("quick_share_mode", true) != false && !isPlaylistUrl
