    ...
    override fun setupDialog(dialog: Dialog, style: Int) {
        ...
        lifecycleScope.launch {
            formatViewModel.formats.collectLatest {
                ...
                // Group by quality height for video, bitrate for audio
                // to keep the list manageable (instead of 30+ entries)
                formats = groupSimilarQualities(it)
                adapter.submitList(formats.toMutableList())
...
//        }
    ...
    private fun groupSimilarQualities(allFormats: List<FormatRecyclerView>): List<FormatRecyclerView> {
        // Bucket by quality key (height for video, bitrate for audio).
        // Within each bucket, keep only the format with the largest
        // filesize + its generic placeholder (for legacy options).
        // This collapses the 30+ entries yt-dlp returns into a short
        // list of distinct qualities — same idea as the Quick Share
        // Sheet's buildQualityOptions, but driven from the FormatRecyclerView
        // model that this dialog consumes.
        val videoBuckets = mutableMapOf<Int, MutableList<FormatRecyclerView>>()
        val audioBuckets = mutableMapOf<Int, MutableList<FormatRecyclerView>>()
        val passthrough = mutableListOf<FormatRecyclerView>()

        for (item in allFormats) {
            val fmt = item.format ?: run { passthrough.add(item); continue }
            val isAudio = (fmt.vcodec.isNullOrEmpty() || fmt.vcodec == "none")
            val key = if (isAudio) {
                (fmt.tbr?.replace("k", "")?.toIntOrNull()) ?: 0
            } else {
                fmt.height ?: 0
            }
            val bucket = if (isAudio) audioBuckets else videoBuckets
            bucket.getOrPut(key) { mutableListOf() }.add(item)
        }

        val deduped = mutableListOf<FormatRecyclerView>()
        for ((_, bucket) in videoBuckets) {
            // Within each height bucket, keep the largest (by filesize) +
            // any header/placeholder items (label != null, format == null)
            val withFormat = bucket.filter { it.format != null }
            val headers = bucket.filter { it.format == null }
            deduped.addAll(headers)
            withFormat.maxByOrNull { it.format!!.filesize }?.let { deduped.add(it) }
        }
        for ((_, bucket) in audioBuckets) {
            val withFormat = bucket.filter { it.format != null }
            val headers = bucket.filter { it.format == null }
            deduped.addAll(headers)
            withFormat.maxByOrNull { it.format!!.filesize }?.let { deduped.add(it) }
        }
        deduped.addAll(passthrough)

        // Sort: video by height desc, audio by bitrate desc, keep stable for
        // the rest so the user's order is preserved
        return deduped.sortedWith(compareBy(
            { it.format?.height ?: 0 },         // video height desc
            { -(it.format?.tbr?.toIntOrNull() ?: 0) } // audio bitrate desc
        )).reversed()
            .let { stable -> stable } // keep first
    }
