...
> # 1.8.9.3 (2026-07) - personal fork

## Simplified share sheet
- When a URL is shared into the app, show a simple bottom sheet (Video / Audio toggle + Quality + Container dropdowns + single Download button) instead of the full 3-tab download card
- New preference `Simplified share sheet` (on by default) to restore the full UI when desired
- "More options" inside the quick sheet reopens the same dialog in full mode

## yt-dlp update channel
- Library `youtubedl-android` already at latest 0.18.1 (no newer upstream release)
- Default yt-dlp update channel changed from STABLE to NIGHTLY so new
  installs pull the absolute latest yt-dlp on first run
- Affects: UpdateUtil, UpdateSettingsModule, UiUtil fallbacks

## App rename to MWYD
- AndroidManifest `android:label` changed from `MWYD` to `MWYD`
- applicationId changed from `com.monestw-cell.ytdl` to `app.mwyd.android`
- ⚠ This installs as a separate app from any existing MWYD install
- Kotlin/Java package (`com.monestw-cell.ytdl.*`) kept unchanged to avoid
  touching 200+ source files. Only the manifest applicationId changed

## MWYD visual identity
- New theme `Theme.Mwyd` with custom Material 3 color palette:
  Teal (primary #0D9488) + Coral (secondary #F97066) + Lime
  (tertiary #84CC16). Distinct from all 7 built-in themes
- Set as default accent in ThemeUtil + general_preferences
- BaseTheme: dialogCornerRadius 28dp -> 32dp
- New card style `Widget.MWYD.CardView` with 24dp rounded corners
- New button style `Widget.MWYD.Button` with 20dp rounded corners
...
The package apks are simply empty apps with jniLibs folder in them containing libxxx.so and libxxx.zip.so. libxxx.zip.so are extracted within the mwyd internal directory, and libxxx.so is accessible throughout the android system.
...
For anyone who is using intent arguments in MWYD for automation, the COMMAND argument has been removed. This is due to a found security vulnerability by SonarPaul where user information and app integrity could be compromised. Read the pdf for more information in the MWYD Updates Telegram Channel.
...
- organized backups to their respective folders inside the MWYD folder. This includes logs, settings backups, and cookie exports
...
- Fix App crashing with exception "com.monestw-cell.ytdl.database.viewmodel.DownloadViewModel.checkIfAllProcessingItemsHaveSameType(Unknown Source:21)..."
...
changed folder structure to mwyd instead of ytdl
